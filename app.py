...
def main():
    ...
    page = st.sidebar.radio(label="", options=["Download Sample file format","Loaded file preview", "Visualize loaded data", "Predictive Modelling"])
   ...
   #____________________________________________________________________________________________
        ...
        if 'Result' in data.columns:
            ...
            elif plots == "Fraud Counts - Admission type":
                fig, ax = plt.subplots(1,1, figsize=(6,3))
                st.subheader('Fraud details w.r.t. Area Service')
                st.write(sns.countplot(data.Admission_type, hue=data.Result,))
                plt.xticks(rotation = 45)
                st.pyplot()
   ...
   #____________________________________________________________________________________________
        ...
        if 'Result' in dataset.columns:
            ...
            st.write(sns.heatmap(cm, annot=True, fmt='.8g'))
            st.pyplot()
